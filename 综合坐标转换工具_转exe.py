    ...
    try:
        ...
        if params.get('sj_direction'):
            ...
            if mode == '3':  # WGS84 → 思极: 需要先转高德再转思极
                ...
                batch_convert(input_file, temp_file, wgs84_to_gcj02, 'WGS84', '高德')
                # 2. 再将高德转为思极
                ...
                if converter.load_model('sj_to_gaode'):
                    # 1. 先将思极转为高德
                    temp_file = os.path.join(base_path, 'run', f"{filename_without_ext}-temp.xlsx")
                    batch_convert(input_file, temp_file, None, '思极', '高德', converter)
                    # 2. 再将高德转为百度
                    batch_convert(temp_file, output_file, gcj02_to_bd09, '高德', '百度')
                    print(f"转换完成！结果已保存到: {output_file}")
                    # 删除临时文件
                    if os.path.exists(temp_file):
                        os.remove(temp_file)
                ...
                if converter.load_model('sj_to_gaode'):
                    ...
                    batch_convert(input_file, temp_file, None, '思极', '高德', converter)
                    # 2. 再将高德转为WGS84
                    batch_convert(temp_file, output_file, gcj02_to_wgs84, '高德', 'WGS84')
                    print(f"转换完成！结果已保存到: {output_file}")
                    # 删除临时文件
                    if os.path.exists(temp_file):
                        os.remove(temp_file)
                ...
                # 1. 先将百度转为高德
                temp_file = os.path.join(base_path, 'run', f"{filename_without_ext}-temp.xlsx")
                batch_convert(input_file, temp_file, bd09_to_gcj02, '百度', '高德')
                # 2. 再将高德转为思极
                if converter.load_model('gaode_to_sj'):
                    batch_convert(temp_file, output_file, None, '高德', '思极', converter)
                    print(f"转换完成！结果已保存到: {output_file}")
                ...
                # 直接转换
                if converter.load_model(params['sj_direction']):
